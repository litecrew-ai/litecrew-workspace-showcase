# RESULT archive 1 (run-log history, moved 2026-08-29)

Moved verbatim from RESULT.md on 2026-08-29 (Task blog-render-profile-fix)
when RESULT.md reached 99.6KB of the 100KB publication gate. Nothing was
deleted; history moved. Edit boundaries: this archive is frozen history --
append new run entries to RESULT.md only.

Rotation policy (also documented in RESULT.md): when RESULT.md exceeds
~60KB, move all but the newest verification-methods section and the last
~10 run entries into the next docs/result-log/archive-<N>.md. Every file
stays under the 100KB text gate; run.py prints a rotation note once the
threshold is passed.
